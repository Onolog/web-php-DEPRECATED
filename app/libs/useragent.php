...
// $Id: Detect.php,v 1.25 2007/04/23 21:24:39 jrust Exp $
...
class Net_UserAgent_Detect {
     ...
     */
    static function &singleton($in_userAgent = null, $in_detect = null) {
        static $instance;
    ...
    static function detect($in_userAgent = null, $in_detect = null) {
        ...
        $browser = array_flip(array('ns', 'ns2', 'ns3', 'ns4', 'ns4up', 'nav', 'ns6', 'belowns6', 'ns6up', 'firefox', 'firefox0.x', 'firefox1.x', 'firefox1.5', 'firefox1up', 'firefox2.x', 'firefox2up', 'firefox3.x', 'firefox3up', 'gecko', 'ie', 'ie3', 'ie4', 'ie4up', 'ie5', 'ie5_5', 'ie5up', 'ie6', 'belowie6', 'ie6up', 'ie7', 'belowie7', 'ie7up', 'opera', 'opera2', 'opera3', 'opera4', 'opera5', 'opera6', 'opera7', 'opera8', 'opera9', 'opera5up', 'opera6up', 'opera7up', 'belowopera8', 'opera8up', 'opera9up', 'operamini', 'aol', 'aol3', 'aol4', 'aol5', 'aol6', 'aol7', 'aol8', 'webtv', 'aoltv', 'tvnavigator', 'hotjava', 'hotjava3', 'hotjava3up', 'konq', 'safari', 'safari1', 'safari2', 'safari2up', 'safari3', 'safari3up', 'netgem', 'webdav', 'icab', 'chrome', 'sonycom2'));
        ...
        // Some versions of firefox are called "iceweasel" or other codenames
        // for trademark, build, and other reasons, but all use the gecko
        // engine. TODO: detect gecko instead.
        $firefox_agents = Net_UserAgent_Detect::get_gecko_variants();
        if (strpos($agt, 'firefox') === false && is_array($firefox_agents)) {
          // only replace if it doesn't already contain "firefox"
          $agt = str_ireplace($firefox_agents, 'firefox', $agt);
        }
        ...
        if ($detectFlags[NET_USERAGENT_DETECT_ALL] || $detectFlags[NET_USERAGENT_DETECT_BROWSER]) {
            ...
            // safari
            if ($browser['safari'] && preg_match('/Safari\/([0-9]*)/i', $agt, $matches)) {
              $safari_version = intval($matches[1]);
              $browser['safari1'] = $safari_version < 400;
              $browser['safari2up'] = $safari_version >= 400;
              $browser['safari3up'] = $safari_version >= 500;
              $browser['safari2'] = $browser['safari2up'] && !$browser['safari3up'];
              $browser['safari3'] = $browser['safari3up'];
            } else {
              $browser['safari1'] = $browser['safari2'] = $browser['safari2up'] = $browser['safari3'] = $browser['safari3up'] = false;
            }
            ...
            $browser['firefox1up'] = $browser['firefox'] && !$browser['firefox0.x'];
            $browser['firefox2.x'] = $browser['firefox'] && strpos($agt, 'firefox/2.') !== false;
            $browser['firefox2up'] = $browser['firefox1up'] && !$browser['firefox1.x'] && !$browser['firefox1.5'];
            $browser['firefox3.x'] = $browser['firefox'] && strpos($agt, 'firefox/3.') !== false;
            $browser['firefox3up'] = $browser['firefox2up'] && !$browser['firefox2.x'];
            ...
            $browser['belowie6']= $browser['ie'] && !$browser['ie6up'] && !$browser['ie6'];
            $browser['belowie7']= $browser['ie'] && !$browser['ie7up'] && !$browser['ie7'];
            ...
            $browser['operamini'] = $browser['opera'] && strpos($agt, 'mini') !== false;
            ...
            // Sony/COM2
            $browser['sonycom2'] = strpos($agt, 'sony/com2') !== false;
        }
        ...
        if ($detectFlags[NET_USERAGENT_DETECT_ALL] ||
            ...
            if ($browser['gecko']) {
                ...
                Net_UserAgent_Detect::setFeature('gecko', isset($matches[1])?$matches[1]:null);
            }
     ...
     */
    static function setOption($in_field, $in_value) {
        $options = &Net_UserAgent_Detect::_getStaticProperty('options');
     ...
     */
    static function isBrowser($in_match) {
     ...
     */
    static function getBrowserString($in_vendorStrings = null) {
     ...
     * @return string first flag that matches
     */
    static function getOS($in_expectList) {
        Net_UserAgent_Detect::detect();
        $os = &Net_UserAgent_Detect::_getStaticProperty('os');
     ...
     */
    static function getOSString($in_osStrings = null) {
     ...
     */
    static function hasQuirk($in_quirk) {
     ...
     */
    static function getQuirk($in_quirk) {
        Net_UserAgent_Detect::detect();
        $quirks = &Net_UserAgent_Detect::_getStaticProperty('quirks');
     ...
     */
    static function hasFeature($in_feature) {
     ...
     */
    static function getFeature($in_feature) {
        Net_UserAgent_Detect::detect();
        $features = &Net_UserAgent_Detect::_getStaticProperty('features');
     ...
     */
    static function getAcceptType($in_expectList, $in_type) {
        Net_UserAgent_Detect::detect();
     ...
     */
    static function hasAcceptType($in_value, $in_type) {
    ...
    static function get_gecko_variants() {
      return array(
        0 => 'bonecho',
        1 => 'granparadiso',
        2 => 'minefield',
